...
It is an official HP-UX template. It requires Zabbix agent 6.4 or newer.
...
|HP-UX: Get filesystems|<p>The `vfs.fs.get` key acquires raw information set about the file systems. Later to be extracted by preprocessing in dependent items.</p>|Zabbix agent|vfs.fs.get|
...
|{#FSNAME}: Free inodes is less than 20%||`last(/HP-UX by Zabbix agent/vfs.fs.dependent.inode[{#FSNAME},pfree])<20`|Warning||
|{#FSNAME}: Free disk space is less than 20%||`last(/HP-UX by Zabbix agent/vfs.fs.dependent.size[{#FSNAME},pfree])<20`|Warning||
